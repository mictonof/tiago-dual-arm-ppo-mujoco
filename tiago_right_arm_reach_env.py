...
class TiagoRightArmReachEnv(gym.Env):
    ...
    def __init__(
        ...
        self.right_finger_subtrees = [
            self._descendant_body_ids(self.right_finger_body_ids[0]),
            self._descendant_body_ids(self.right_finger_body_ids[1]),
        ]
    ...
    def _gripper_base_pos(self):
        return self.data.xpos[self.right_wrist_body_id].copy()
    ...
    def _gripper_action_center_pos(self):
        right_tip = self._farthest_geom_point_on_subtree(self.right_finger_subtrees[0])
        left_tip  = self._farthest_geom_point_on_subtree(self.right_finger_subtrees[1])
        return 0.5 * (right_tip + left_tip)
