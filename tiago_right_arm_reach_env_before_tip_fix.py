...
class TiagoRightArmReachEnv(gym.Env):
    ...
    def __init__(
        ...
        # Apertura visual del gripper
        self.gripper_open = 0.03

        # Distancia desde el centro del gripper hasta la punta virtual.
        # Si todavía queda corto, prueba 0.16.
        # Si se pasa demasiado, prueba 0.12.
        self.tip_offset = 0.14
    ...
    def _ee_pos(self):
        ...
        # Muñeca / inicio del gripper
        wrist = self.data.xpos[self.right_wrist_body_id].copy()

        # Dirección desde la muñeca hacia el gripper
        direction = mid - wrist
        ...
        # Punta virtual al final del gripper
    ...
    def _sample_goal(self):
        ...
        zones = ["front_mid_center", "front_far_center", "front_from_ee"]
        probs = [0.45, 0.35, 0.20]

        for _ in range(500):
            zone = self.np_random.choice(zones, p=probs)

            if zone == "front_mid_center":
                g = torso + np.array([
                    self.np_random.uniform(0.24, 0.30),
                    self.np_random.uniform(-0.08, -0.02),
                    self.np_random.uniform(-0.20, -0.10),
                ])
            elif zone == "front_far_center":
                g = torso + np.array([
                    self.np_random.uniform(0.28, 0.34),
                    self.np_random.uniform(-0.09, -0.03),
                    self.np_random.uniform(-0.22, -0.12),
                ])
            else:
                g = ee0 + np.array([
                    self.np_random.uniform(0.10, 0.16),
                    self.np_random.uniform(-0.04, 0.02),
                    self.np_random.uniform(-0.12, -0.04),
                ])
            ...
            if not (0.08 <= dist_ee <= 0.20):
                continue
            if not (0.18 <= dist_shoulder <= 0.42):
                continue
            if not (0.22 <= dist_torso <= 0.36):
                continue
            if not (0.35 <= g[2] <= 0.75):
                continue
            if g[0] <= torso[0] + 0.08:
                continue
        ...
        return (torso + np.array([0.28, -0.05, -0.16], dtype=np.float64)).astype(np.float64)
