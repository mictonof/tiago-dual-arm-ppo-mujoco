...
class TiagoRightArmReachEnv(gym.Env):
    ...
    def _sample_goal(self):
        ...
        for _ in range(500):
            ...
            if zone == "front_right_mid":
                g = torso + np.array([
                    ...
                    self.np_random.uniform(-0.45, -0.30),
                ])
            elif zone == "front_right_far":
                g = torso + np.array([
                    self.np_random.uniform(0.30, 0.36),
                    self.np_random.uniform(-0.36, -0.24),
                    self.np_random.uniform(-0.50, -0.34),
                ])
            else:
                g = ee0 + np.array([
        ...
        return (torso + np.array([0.30, -0.12, -0.38], dtype=np.float64)).astype(np.float64)
